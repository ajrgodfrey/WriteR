# 2022.11.29 This file is for insertion of anything mathematical, so should be useful across implementations
...
def OnDoubleSummation(self, event):
    self.editor.WriteText(
        "\\sum_{ lower }^{ upper }{\\sum_{ lower }^{ upper }{ what }}"
    )


def OnDoubleIntegral(self, event):
    self.editor.WriteText(
        "\\int_{ lower }^{ upper }{\\int_{ lower }^{ upper }{ what }}"
    )
